...
def return_decrypt_location_response_locations(device_update_protobuf):
    ...
    for loc in location_time_array:
        ...
        if loc.status == Common_pb2.Status.SEMANTIC:
            ...
            latitude = proto_loc.latitude / 1e7
            longitude = proto_loc.longitude / 1e7
            locitem = {"lat": latitude,"lng":longitude}
            
        if locitem:
            locitem["timestamp"] = datetime.datetime.fromtimestamp(loc.time).isoformat()

        locs.append(locitem)
    print("[LocationRequest] Served Location Request")
    return locs
